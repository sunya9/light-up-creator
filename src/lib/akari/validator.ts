...
function getAdjacentWhiteWithBulb(grid: PuzzleDef, pos: Pos, bulbs: Set<string>): number {
  let count = 0;
  for (const d of ADJACENT) {
    const r = pos.row + d.row;
    const c = pos.col + d.col;
    if (r >= 0 && r < grid.rows && c >= 0 && c < grid.cols) {
      if (grid.cells[r][c].kind === "white" && bulbs.has(posToKey({ row: r, col: c }))) {
        count++;
      }
    }
  }
...
/** 完成した解を検証する */
export function validate(grid: PuzzleDef, bulbs: Set<string>): ValidationResult {
  ...
  for (let r = 0; r < grid.rows; r++) {
    for (let c = 0; c < grid.cols; c++) {
      if (grid.cells[r][c].kind === "white" && !lit.has(posToKey({ row: r, col: c }))) {
  ...
  for (let r = 0; r < grid.rows; r++) {
    for (let c = 0; c < grid.cols; c++) {
      ...
      if (cell.kind === "black" && cell.number !== null) {
        const actual = getAdjacentWhiteWithBulb(grid, { row: r, col: c }, bulbs);
