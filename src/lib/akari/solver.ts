...
function initState(def: PuzzleDef): SolverState {
  ...
  const numberConstraints = new Map<string, { number: number; neighbors: string[] }>();
  ...
  return { def, assignment, whiteCells, numberConstraints, illuminationPaths, illuminators };
}

function cloneAssignment(assignment: Map<string, SolverCell>): Map<string, SolverCell> {
  return new Map(assignment);
}

type PropagateResult = { ok: true; assignment: Map<string, SolverCell> } | { ok: false };
...
function propagateState(state: SolverState, assignment: Map<string, SolverCell>): PropagateResult {
  let changed = true;
  while (changed) {
    changed = false;
    ...
    for (const key of state.whiteCells) {
      if (isIlluminatedWithAssignment(key, assignment, state.illuminationPaths)) continue;
...
function isSolved(state: SolverState, assignment: Map<string, SolverCell>): boolean {
  for (const key of state.whiteCells) {
    const val = assignment.get(key)!;
    if (val === "undecided") return false;
    if (!isIlluminatedWithAssignment(key, assignment, state.illuminationPaths)) return false;
  }
  return true;
}

function chooseBranchCell(state: SolverState, assignment: Map<string, SolverCell>): string | null {
