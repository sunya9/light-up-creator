...
test("validator: validates a correct simple solution", () => {
...
test("validator: detects unlit white cells", () => {
...
test("validator: detects bulbs illuminating each other", () => {
  const grid = parsePuzzle(["...", "...", "..."]);
...
test("validator: detects number constraint violation (too many bulbs)", () => {
  // 1 の黒マスに隣接 bulb が2つ
...
test("validator: detects number constraint violation (too few bulbs when all placed)", () => {
...
test("validator: allows bulbs on same line separated by black cell", () => {
  const grid = parsePuzzle([".#."]);
  const bulbs = new Set(["0,0", "0,2"]);
  const result = validate(grid, bulbs);
  expect(result.valid).toBe(true);
